...
namespace TowerSoft.SteamAchievs.Cron.Jobs {
    public class FullGameSync {
        ...
        public FullGameSync(UnitOfWork uow, SteamApiClient steamApi, SteamGridClient steamGridClient, ProtonDbService protonDbService,
            HowLongToBeatService howLongToBeatService, IOptions<AppSettings> appSettings, ILogger<FullGameSync> logger) {
            this.uow = uow;
        ...
        public void Run() {
            ...
            List<UserAppModel> userAppModels = JsonConvert.DeserializeObject<List<UserAppModel>>(File.ReadAllText("tempData.json"));
        ...
        private void SyncSteamCategories(List<UserAppModel> userAppModels) {
            ...
            SteamCategoryNameComparer comparer = new SteamCategoryNameComparer();
        ...
        private void SyncSteamUserTags(List<UserAppModel> userAppModels) {
            ...
            SteamUserTagNameComparer comparer = new SteamUserTagNameComparer();
            var dbEntities = repo.GetAll().ToHashSet(comparer);
        ...
        private List<UserAppModel> GetOwnedAppData() {
            ...
            foreach (OwnedApp ownedApp in ownedApps.OrderBy(x => x.Name)) {
                ...
                //Task<SteamApp> steamAppTask = steamApi.StoreClient.GetSteamApp(ownedApp.SteamAppID);
                //Task<List<GlobalAchievementStat>> globalAchievementsTask = steamApi.UserStatsClient.GetGlobalAchievementStats(ownedApp.SteamAppID);
                //Task<GameStatsSchema> gameStatsSchemaTask = steamApi.UserStatsClient.GetGameStatsSchema(ownedApp.SteamAppID);
                //Task<DeckCompatibility> deckCompatibilityTask = steamApi.StoreClient.GetDeckCompatibility(ownedApp.SteamAppID);
                //Task<ReviewSummary> reviewSummaryTask = steamApi.StoreClient.GetReviews(ownedApp.SteamAppID);
                //Task.WhenAll(new List<Task> { steamAppTask, globalAchievementsTask, gameStatsSchemaTask, deckCompatibilityTask, reviewSummaryTask });
                ...
                if (string.IsNullOrWhiteSpace(steamApp.Type) || steamApp.Type == "game" || steamApp.Type == "advertising") {
                    //HACK Better type check needed. I think only filtering out 'mod' in needed
                } else {
                    logger.LogWarning($"{steamApp.Name} type is '{steamApp.Type}' instead of game.");
                    count++;
                    continue;
                }
